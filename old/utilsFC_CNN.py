...
def train_epoch(model: nn.Module, device: torch.device, train_loader: DataLoader, 
    ...
    for batch_idx, (dataCNN, dataFCNN, target) in enumerate(train_loader):
        dataCNN, dataFCNN, target = dataCNN.to(device), dataFCNN.to(device), target.to(device)
        optimizer.zero_grad()
        output = model(dataCNN, dataFCNN)
...
def eval_epoch(model: nn.Module, device: torch.device, val_loader: DataLoader, 
    ...
    with torch.no_grad():
        for dataCNN, dataFCNN, target in val_loader:
            dataCNN, dataFCNN, target = dataCNN.to(device), dataFCNN.to(device), target.to(device)
            output = model(dataCNN, dataFCNN)
            loss = criterion(output, target)
            val_loss += loss.item()
...
def evaluate_model(model, testloader, device):
    ...
    total = 0
    with torch.no_grad():
        for dataCNN, dataFCNN, labels in testloader:
            dataCNN, dataFCNN, labels = dataCNN.to(device), dataFCNN.to(device), labels.to(device)
            outputs = model(dataCNN, dataFCNN)
